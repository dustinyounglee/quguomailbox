  ...
  componentDidMount() {
    ...
    this._server.once('error', (err) => {
      AppEnv.showErrorDialog({
        ...
        message: localized(
          `To listen for the Gmail Oauth response, QGMail needs to start a webserver on port ${LOCAL_SERVER_PORT}. Please go back and try linking your account again. If this error persists, use the IMAP/SMTP option with a Gmail App Password.\n\n%@`,
          err
        ),
      });
    });
    ...
    if (authStage === 'accountSuccess') {
      return (
        <div>
          ...
          <h3>{localized('Adding your account to QGMail…')}</h3>
        </div>
      );
    }
