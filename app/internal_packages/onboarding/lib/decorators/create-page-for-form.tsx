    ...
    onConnect = (updatedAccount?: Account) => {
      ...
      if (
        ...
        const buttonIndex = require('@electron/remote').dialog.showMessageBoxSync({
          ...
          detail: localized(
            `This looks like a Gmail account! While it's possible to setup an App Password and connect to Gmail via IMAP, QGMail also supports Google OAuth. Go back and select "Gmail & Google Apps" from the provider screen.`
          ),
        });
        if (buttonIndex === 0) {
